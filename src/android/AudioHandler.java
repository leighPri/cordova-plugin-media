...
public class AudioHandler extends CordovaPlugin {
    ...
    public void startPlayingAudio(String id, String file) {
        ...
        getAudioFocus();
        /* User removes battery optimization popup */
// we're going to check, then ask for optimization permission
if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
  Context context = cordova.getActivity().getApplicationContext();

  SharedPreferences settings = context.getSharedPreferences("BatteryOptimizationPrompt", 0);
  boolean prompted = settings.getBoolean("prompted", false);
  ...
  if (!prompted) {
      ...
      if (!pm.isIgnoringBatteryOptimizations(packageName)) {
          ...
          editor.putBoolean("prompted", true);
          editor.commit();
      }
  }
}
